...
export default function Sidebar({
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-50 flex flex-col bg-white dark:bg-primary-900 border-r border-light-border dark:border-none transition-all duration-300 ease-in-out md:relative ${collapsed ? "w-20" : "w-64"} ${mobileOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}
      >
        {/* Logo area */}
          ...
          <div className="flex items-center gap-3">
            ...
            {!collapsed && (
              <div className="flex flex-col">
                <span className="text-lg font-bold leading-none tracking-tight text-primary-900 dark:text-white">
                  IMS
                </span>
        ...
        <div className="flex-1 overflow-y-auto py-4 custom-scrollbar">
          {MENU_SECTIONS.filter((section) => !section.adminOnly || isAdmin).map(
            (section, index) => (
                    ...
                    .map((item, itemIndex) => {
                      ...
                      return (
                        <li key={itemIndex}>
                          ...
                          >
                            <Icon
                              className={`${collapsed ? "mx-auto" : "mr-3"} h-5 w-5 shrink-0`}
                            />
